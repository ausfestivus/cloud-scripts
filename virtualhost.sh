...
# credit: https://github.com/RoverWire/virtualhost/blob/master/virtualhost.sh
...
apacheUser=$(ps -ef | grep -E '(httpd|apache2|apache)' | grep -v root | head -n1 | awk '{print $1}')
email='festivus\+webmaster\@gmail.com'
...
if [ "$action" == 'create' ]; then
	...
	cat <<-APACHEVHOST >"$sitesAvailabledomain"
		<VirtualHost *:80>
			ServerAdmin $email
			ServerName $domain
			ServerAlias $domain
	...
	if [ -e /mnt/c/Windows/System32/drivers/etc/hosts ]; then
		if ! echo -e "\r127.0.0.1       $domain" >>/mnt/c/Windows/System32/drivers/etc/hosts; then
